...
/**
 * Frontend was built standalone and calls a bunch of legacy paths.
 * This router maps them to the new orchestration where possible
 * and returns benign defaults otherwise so nothing 404s.
 */
export function createLegacyAliasRouter(): Router {
  const r = Router();

  // NOTE: /api/project-status, /api/tunnel-info, /api/run-project,
  // /api/stop-project, /api/restart are owned by createRuntimeRouter()
  // and back real child processes — they were intentionally removed here.
  ...
  // ── filesystem conflict aliases (no-op safe defaults) ──────────────
  ...
  // ── agent-ultra-service (frontend uses /api/agent/run/*) ───────────
  ...
  // simple state machine for ultra runs (in-memory)
  ...
  // legacy /api/run/* aliases kept for backward compat
  ...
  // ── file undo / history / conflict-check (agent-ultra-service) ─────
  ...
  r.post("/api/file/undo", async (req: Request, res: Response) => {
    ...
    try {
      const projectId = await resolveProjectId(req);
      // Find the most recent applied diff for this file and revert to its oldContent
      const rows = await db
        .select()
        .from(diffQueue)
  ...
  r.post("/api/file/conflict-check", async (req: Request, res: Response) => {
    ...
    try {
      const projectId = await resolveProjectId(req);
      // Conflict = a newer applied diff exists for this file beyond baseVersionId
      const rows = await db
        .select()
        .from(diffQueue)
        .where(and(eq(diffQueue.projectId, projectId), eq(diffQueue.filePath, filePath)))
