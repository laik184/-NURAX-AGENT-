...
/**
 * Real runtime endpoints — these replace the previous stubs and
 * actually spawn child processes inside `.sandbox/<projectId>/`.
 */
export function createRuntimeRouter(): Router {
  ...
  // ── Real package manager (npm install / uninstall / list / run) ────
  ...
  // ── Screenshot of running preview (HTML + headers snapshot) ────────
  // We don't ship a headless browser — but we can fetch and return a
  // structured snapshot of the running page so agents/tools can inspect it.
  r.get("/api/screenshot", async (req: Request, res: Response) => {
    try {
      ...
      // Extract <title> and a text excerpt as a lightweight "visual" hint
