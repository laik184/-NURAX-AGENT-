...
import { projectRoot, ensureProjectDir } from "../sandbox/sandbox.util.ts";
...
// SECURITY: only these binaries are allowed via /api/solo-pilot/execute.
// Anything else returns 403. The list is intentionally narrow — UI features
// that need other commands should add a dedicated, validated route.
...
// SECURITY: arg-level pattern that disallows shell metacharacters so callers
// can't smuggle "rm -rf /" via args.
const ARG_DENY_RE = /[;&|`$<>(){}\\]|\$\(/;
...
function validateExec(command?: string, args?: string[]): { ok: boolean; reason?: string } {
  ...
  // Disallow path traversal in the binary itself
...
export function createSoloPilotRouter(): Router {
  ...
  r.post("/execute", async (req: Request, res: Response) => {
    ...
    // SECURITY: shell:false — args are passed as a real argv array, no shell parsing.
