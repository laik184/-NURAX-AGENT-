...
export const gitAdd: Tool = {
  ...
  parameters: {
    type: "object",
    properties: { paths: { type: "array", items: { type: "string" }, description: "Files/directories to stage (default: all)" } },
  },
...
export const gitCommit: Tool = {
  ...
  async run(args, ctx: ToolContext): Promise<ToolResult> {
    const projectDir = getProjectDir(ctx.projectId);
    await ensureGitRepo(projectDir);
    if (args.stage_all) await runGit(["add", "."], projectDir, ctx.signal);
    const gitArgs = ["-c", "user.email=agent@nura-x.dev", "-c", "user.name=NURA-X Agent", "commit", "-m", args.message as string];
...
export const gitClone: Tool = {
  ...
  async run(args, ctx: ToolContext): Promise<ToolResult> {
    const projectDir = getProjectDir(ctx.projectId);
    const cloneArgs  = ["clone", args.url as string, ...(args.directory ? [args.directory as string] : [])];
    const { ok, stdout, stderr, exitCode } = await runGit(cloneArgs, projectDir, ctx.signal);
    return { ok, result: { cloned: ok, url: args.url, stdout, exitCode }, error: ok ? undefined : stderr };
  },
};
...
export const gitPush: Tool = {
  ...
  async run(args, ctx: ToolContext): Promise<ToolResult> {
    const projectDir = getProjectDir(ctx.projectId);
    const gitArgs    = ["push", (args.remote as string) || "origin", ...(args.branch ? [args.branch as string] : []), ...(args.force ? ["--force"] : [])];
    const { ok, stdout, stderr, exitCode } = await runGit(gitArgs, projectDir, ctx.signal);
...
export const gitPull: Tool = {
  ...
  async run(args, ctx: ToolContext): Promise<ToolResult> {
    const projectDir = getProjectDir(ctx.projectId);
    const gitArgs    = ["pull", (args.remote as string) || "origin", ...(args.branch ? [args.branch as string] : [])];
    const { ok, stdout, stderr, exitCode } = await runGit(gitArgs, projectDir, ctx.signal);
