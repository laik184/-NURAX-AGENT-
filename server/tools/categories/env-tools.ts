...
export const envRead: Tool = {
  ...
  parameters: {
    type: "object",
    properties: { path: { type: "string", description: "Path to .env file (default: .env)" } },
  },
  async run(args, ctx: ToolContext): Promise<ToolResult> {
    ...
    try {
      ...
      return { ok: true, result: { path: args.path || ".env", count: Object.keys(env).length, env: maskSecrets(env) } };
    } catch (e: any) {
      if (e.code === "ENOENT") return { ok: true, result: { path: args.path || ".env", count: 0, env: {}, message: ".env file does not exist yet." } };
...
export const envWrite: Tool = {
  name: "env_write",
  description: "Set or update a key in the project .env file. Creates the file if it doesn't exist.",
  parameters: {
    type: "object",
    properties: {
  ...
  async run(args, ctx: ToolContext): Promise<ToolResult> {
    ...
    try {
      ...
      else { if (lines.length > 0 && lines[lines.length - 1] !== "") lines.push(""); lines.push(newLine); }
