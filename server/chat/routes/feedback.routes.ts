...
export function createChatFeedbackRouter(): Router {
  ...
  router.post("/messages/:id/feedback", async (req, res) => {
    ...
    if (!VALID_FEEDBACK.has(feedback)) {
      return res.status(400).json({ ok: false, error: `feedback must be: ${[...VALID_FEEDBACK].join(" | ")}` });
    }
    try {
      const [updated] = await db.update(chatMessages).set({ feedback }).where(eq(chatMessages.id, id)).returning();
      if (!updated) return res.status(404).json({ ok: false, error: "message not found" });
  ...
  router.delete("/messages/:id/feedback", async (req, res) => {
    ...
    if (!id || isNaN(id)) return res.status(400).json({ ok: false, error: "valid message id required" });
    try {
      const [updated] = await db.update(chatMessages).set({ feedback: null }).where(eq(chatMessages.id, id)).returning();
      if (!updated) return res.status(404).json({ ok: false, error: "message not found" });
